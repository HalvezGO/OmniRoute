...
Live count: `ls src/lib/db/*.ts | wc -l` (currently 95). Migrations: `ls src/lib/db/migrations/*.sql | wc -l` (currently 131).
...
- **`db/migrations/`** — 131 SQL files (`001_initial_schema.sql` → `134_*.sql`). Each runs in a transaction, never fails partially.
